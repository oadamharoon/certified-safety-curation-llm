#!/bin/bash
# E17: E10d with the Qwen-7B judge score (certstrat2_qwen800), for the 1:1 scorer comparison (advisor comment C030):
# harmful-prompt stratum alpha_h = 0.20, benign-prompt stratum alpha_b = 0.15, delta/2 each,
# n = 800, Qwen2.5-7B-Instruct judge scorer. Overall bound = mixture of the two targets by selection shares.
PY="${PY:-python}"
set -u; W="${CDC_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)}"; PY=${PY}; cd $W
env PYTHONNOUSERSITE=1 $PY - <<'PY' 2>&1 | grep -v -i warn | tee logs/e17_select.log
...
g = np.array(json.load(open("results/scores_Qwen2.5-7B-Instruct_N4000_u0.25.json")))
...
for s in range(3):
    ...
    meta = {"certified": ok, "overall_bound": float(bound), "alpha_h": .20, "alpha_b": .15, "n": len(idx), "unsafe_frac": float(unsafe[idx].mean()), "safety_demo_share": float(demo[idx].mean()), "scorer": "Qwen2.5-7B-Instruct"}
    json.dump({"idx": idx, "meta": meta}, open(f"selections/certstrat2_qwen800_s{s}.json", "w")); summ.setdefault("certstrat2_qwen800", {})[str(s)] = meta
    print(f"  certstrat2_qwen800 s{s}: n={len(idx)} unsafe={meta['unsafe_frac']:.3f} (bound {bound:.3f}) demos={meta['safety_demo_share']:.3f} certified={ok}")
json.dump(summ, open("results/step2_selections.json", "w"), indent=1)
PY
names=""
for s in 0 1 2; do env PYTHONNOUSERSITE=1 ARM=certstrat2_qwen800 SEED=$s $PY scripts/step2_sft.py > logs/sft/certstrat2_qwen800_s$s.log 2>&1 && echo "done s$s" || echo "FAIL s$s"; names="$names,certstrat2_qwen800_s$s"; done
env PYTHONNOUSERSITE=1 EVAL_ONLY=${names#,} $PY scripts/step2_eval.py > logs/e17_gen.log 2>&1; env PYTHONNOUSERSITE=1 $PY scripts/step2_eval2.py > logs/e17_eval2.log 2>&1; env PYTHONNOUSERSITE=1 $PY scripts/e1_evaluators.py > logs/e17_mdj.log 2>&1; env PYTHONNOUSERSITE=1 $PY scripts/e1_panel2.py > logs/e17_panel2.log 2>&1; env PYTHONNOUSERSITE=1 $PY scripts/e6_utility.py > logs/e17_e6.log 2>&1
...
for a in ["full","cert400","certstrat_qwen","certstrat2_qwen800","certlg400","certstrat2_lg800","oracle"]:
